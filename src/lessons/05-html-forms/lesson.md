...
:::shot img/forms-input-types.webp width=520
Одно и то же `<input>`, разные `type`. Пароль прячется точками, у number есть стрелки, range — ползунок
:::
...
Ещё несколько типов, которые встретятся в работе:

| type | Что даёт |
|---|---|
| `url` | Проверяет, что введён адрес с `https://` |
| `search` | Поле поиска, на телефоне кнопка «Найти» на клавиатуре |
| `time` | Время: часы и минуты |
| `datetime-local` | Дата и время в одном поле |
| `file` | Выбор файла — об этом ниже |
| `hidden` | Невидимое поле: значение уходит на сервер, человек его не видит |

Неизвестный `type` браузер превращает в обычный `text` — опечатка в типе ничего не сломает, но и пользы не будет.

## Выбор из вариантов

Когда ответов немного, дайте выбрать, а не печатать.

:::shot img/forms-choices.webp width=600
Четыре способа выбора, каждый в своей группе fieldset
:::
...
- `value` — что уйдёт на сервер, если выбран вариант. Текст подписи — для человека, `value` — для программы.
- `checked` — вариант выбран сразу, `selected` — то же для `<option>`.
- Подпись можно не связывать через `for`, а обернуть ею поле: `<label><input type="checkbox" name="syrup"> Сироп</label>`. Щелчок по слову тоже работает.
- Длинный список делят на группы: `<optgroup label="Горячие">` внутри `<select>`.

**Подсказки при вводе.** `<datalist>` показывает варианты, но разрешает написать и свой:

:::sandbox Подсказки datalist height=200
```html
...
Щёлкните в поле или начните печатать «Ка» — появится подсказка. Связь — атрибут `list` у поля и `id` у списка.

## Отправка файлов

Поле `type="file"` открывает выбор файла. `accept` ограничивает типы, `multiple` разрешает несколько файлов:

```html
<form action="/upload" method="post" enctype="multipart/form-data">
  <label for="photo">Фото вашего латте-арта</label>
  <input id="photo" name="photo" type="file" accept="image/*" multiple>
  <button type="submit">Отправить</button>
</form>
```

Файл нельзя передать в адресе, поэтому у формы обязательно `method="post"` и `enctype="multipart/form-data"`. Без `enctype` на сервер уйдёт только имя файла.
...
| `step` | Шаг числа: `step="0.5"` разрешит 1,5 кг, но не 1,3 |

Подсказку к `pattern` пишут в `title`: `pattern="[0-9]{6}" title="Индекс — 6 цифр"`. Браузер покажет её в сообщении об ошибке. Сам шаблон — это регулярное выражение, подробно разберём в модуле про JavaScript.

Ещё три полезных атрибута:

- `autocomplete="name"`, `"tel"`, `"email"` — браузер предложит подставить сохранённые данные. Заполнить форму можно в два щелчка.
- `disabled` — поле серое, его нельзя изменить, и значение **не** уходит на сервер.
- `readonly` — поле нельзя изменить, но значение уходит.
...
**Объединение ячеек.** `colspan="3"` растягивает ячейку на три столбца, `rowspan="2"` — на две строки. Итоговую строку кладут в `<tfoot>`:

:::shot img/forms-table.webp width=420
Голубые ячейки — объединённые: «Объём» на три столбца, «только маленький» на два. «Напиток» — на две строки
:::

```html
<tr><th rowspan="2">Напиток</th><th colspan="3">Объём</th></tr>
<tr><th>S</th><th>M</th><th>L</th></tr>
```

Считайте ячейки: в каждой строке с учётом объединённых должно получиться одинаковое число столбцов, иначе таблица «поедет».
...
- q: Как дать подсказки при вводе, но разрешить написать свой вариант?
  options:
    - "`<select>`"
    - '`<input list="drinks">` и `<datalist id="drinks">`'
    - Несколько radio
    - "`placeholder`"
  answer: 1
  why: datalist предлагает варианты, но поле остаётся текстовым. select разрешает только варианты из списка.
